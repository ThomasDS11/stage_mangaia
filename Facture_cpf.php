...
$content = file_get_contents(dirname(__FILE__).'/facture_cpf.htm'); // Attention au chemin d'accès au fichier template. ici, il est dans le même répertoire que export.php sinon donnez le chemin correct.
...
$filename = "facture_cpf_$id_demande.doc";// Vérifie que l'on peut écrire dans le fichier if(!is_writable($filename)) exit();// Vérifie que l'on peut ouvrir le fichier
