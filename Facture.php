...
$statement_recup_obj = $pdo->prepare("SELECT objectif_pedagogique FROM formation where nom_formation = :nom_formation");
$statement_recup_obj->bindParam(":nom_formation", $nom_formation);
$statement_recup_obj->execute();  
$objectif_formation_res = $statement_recup_obj->fetch(PDO::FETCH_ASSOC);
$objectif_formation = $objectif_formation_res["objectif_pedagogique"];

$statement_recup_form = $pdo->prepare("SELECT nom_formateur FROM formateur where id_formateur = :id_formateur");
$statement_recup_form->bindParam(":id_formateur", $id_formateur1);
$statement_recup_form->execute();  
$objectif_formateur_res = $statement_recup_form->fetch(PDO::FETCH_ASSOC);
$objectif_formateur = $objectif_formateur_res["nom_formateur"];
...
$content = file_get_contents(dirname(__FILE__).'/facture_classique_template.htm'); // Attention au chemin d'accès au fichier template. ici, il est dans le même répertoire que export.php sinon donnez le chemin correct.
...
$content = str_replace('##nom_demande##', $nom_demande, $content);
$content = str_replace('##genre_contact_societe#', $genre_contact_societe, $content);
$content = str_replace('##nom_contact_societe##', $nom_contact_societe, $content);
$content = str_replace('##nom_formateur##', $objectif_formateur, $content);




// La suite du fichier à l'étape 3
$filename = "facture_classique_$id_demande.doc";// Vérifie que l'on peut écrire dans le fichier if(!is_writable($filename)) exit();// Vérifie que l'on peut ouvrir le fichier
